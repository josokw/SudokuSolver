   ...
   if (isAnElementOf(value, SudokuGrid::U))
   {
      ...
      else
      {
         throw std::logic_error("-- Sudoku elements must be in the range "
                                "[0,"
                                + boost::lexical_cast<std::string>(int(SudokuGrid::ORDER2)) + "]");
      }
   }
   else
   {
      if (value != 0)
      {
         throw std::logic_error("-- Sudoku element: " +
                                boost::lexical_cast<std::string>(value) + " must be in the range "
                                "[0," + boost::lexical_cast<std::string>(int(SudokuGrid::ORDER2)) + "]");
      }
      _cell[row][column] = 0;
      isAdded = true;
   }
